...
void loop() { 
  ...
  int ledValue = calcLEDBrightness(distance);
  ...
  // do something here
  delay(0); // Assume that it takes 0ms to do something.
  
  // update last sampling time
  last_sampling_time += INTERVAL;
}

int calcLEDBrightness(float d) {
  if (d <= 100.0 || d >= 300.0) {
    return 255; // 꺼짐
  } 
  else if (d > 100.0 && d <= 200.0) {
    // 100→255, 200→0 (밝아짐)
